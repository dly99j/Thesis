...
namespace spsh {
  ...
  private:
    ...
    [[nodiscard]]
    auto pause_game() -> bool;

    [[nodiscard]]
    auto draw_pause_game() -> std::pair<sf::RectangleShape, sf::RectangleShape>;
    ...
    auto handle_collision(std::vector<projectile> &, std::vector<powerup> &) -> void;

    [[nodiscard]]
    auto handle_lifetime() -> game_state;

    [[nodiscard]]
    auto handle_game_over() -> bool;
    ...
    [[nodiscard]]
    auto generate_random_coords(sf::Vector2u) const -> sf::Vector2f;
