...
auto movable::operator==(const movable& other) const -> bool {
    return this->get_direction() == other.get_direction() &&
           ...
           this->get_reduced_texture_rect() ==
               other.get_reduced_texture_rect() &&
...
auto movable::calculate_move() const -> sf::Vector2f {
    ...
    const auto move_up = [&](const bool is_diag) {
        is_diag ? movement.y -= get_speed() / sqrt_two
                : movement.y -= get_speed();
    };
    const auto move_right = [&](const bool is_diag) {
        is_diag ? movement.x += get_speed() / sqrt_two
                : movement.x += get_speed();
    };
...
auto movable::is_off_map(
    const std::unique_ptr<sf::Vector2u>& t_window_size) const -> bool {
    auto [x, y] = get_position();
    if (x < 0.0f || y < 0.0f) {
        return true;
    }
    if (x + static_cast<float>(get_texture_size().x) >
            static_cast<float>(t_window_size->x) ||
        y + static_cast<float>(get_texture_size().y) >
            static_cast<float>(t_window_size->y)) {
        return true;
    }
    return false;
}
