...
namespace spsh {
chasing_enemy::chasing_enemy(const direction t_direction, const float t_speed,
                             const short t_lives)
...
auto chasing_enemy::calculate_direction(
    ...
    const auto player_middle_x =
        t_player_rect.value().left + t_player_rect.value().width / 2.0f;
    ...
    if (const auto curr_delta = std::abs(enemy_middle_x - player_middle_x);
        curr_delta < delta) {
        ...
        if (change_random_direction()) {
            ...
            if (enemy_middle_x > player_middle_x) {
                if ((get_direction() == direction::up_left ||
                     get_direction() == direction::up_right) &&
                    pos_top > 5.0f) {
                    set_direction(direction::up_left);
                } else if ((get_direction() == direction::down_left ||
                            get_direction() == direction::down_right) &&
                           pos_bot < static_cast<float>(t_screen_dimensions.y) /
                                         3.0f) {
                    set_direction(direction::down_left);
                } else {
                    set_direction(direction::left);
