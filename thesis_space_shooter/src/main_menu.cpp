...
namespace spsh {
    ...
    auto main_menu::display() const -> menu_result {
        ...
        title.setPosition((static_cast<float>(m_window.getSize().x) - title.getGlobalBounds().width) / 2.f, 100.f);
        ...
        play_button.setPosition((static_cast<float>(m_window.getSize().x) - play_button.getSize().x) / 2.f, 250.f);
        choose_map_button.setPosition((static_cast<float>(m_window.getSize().x) - choose_map_button.getSize().x) / 2.f,
                                      400.f);
        quit_button.setPosition((static_cast<float>(m_window.getSize().x) - quit_button.getSize().x) / 2.f, 550.f);

        sf::Text play_text("Start Game", font, 30);
        play_text.setFillColor(sf::Color::White);
        play_text.setPosition(
            play_button.getPosition().x + (play_button.getSize().x - play_text.getGlobalBounds().width) / 2.f,
            play_button.getPosition().y + (play_button.getSize().y - play_text.getGlobalBounds().height) / 2.f - 5.f);

        sf::Text choose_map_text("Choose Map", font, 30);
        choose_map_text.setFillColor(sf::Color::White);
        choose_map_text.setPosition(
            choose_map_button.getPosition().x + (
                choose_map_button.getSize().x - choose_map_text.getGlobalBounds().width) / 2.f,
            choose_map_button.getPosition().y + (
                choose_map_button.getSize().y - choose_map_text.getGlobalBounds().height) / 2.f - 5.f);

        sf::Text quit_text("Quit", font, 30);
        quit_text.setFillColor(sf::Color::White);
        quit_text.setPosition(
            quit_button.getPosition().x + (quit_button.getSize().x - quit_text.getGlobalBounds().width) / 2.f,
            quit_button.getPosition().y + (quit_button.getSize().y - quit_text.getGlobalBounds().height) / 2.f - 5.f);
