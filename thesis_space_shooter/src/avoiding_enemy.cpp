...
namespace spsh {
    ...
    auto avoiding_enemy::is_bullet_close(
        ...
        const auto enemy_middle_x =
                get_reduced_texture_rect().left + get_reduced_texture_rect().width / 2.0f;
        const auto enemy_middle_y =
                get_reduced_texture_rect().top + get_reduced_texture_rect().height / 2.0f;
