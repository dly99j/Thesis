...
namespace spsh {
    ...
    auto avoiding_enemy::left_right_switch(const sf::Vector2u t_screen_dimensions) -> void {
        if (m_is_going_left && get_position().x < 20.0f) {
            m_is_going_left = false;
        }
        if (!m_is_going_left &&
            get_position().x + get_texture_rect().width > static_cast<float>(t_screen_dimensions.x) - 20.0f) {
            m_is_going_left = true;
        }
    }

    auto avoiding_enemy::is_bullet_close(const sf::Vector2u t_screen_dimensions,
                                         const std::optional<sf::FloatRect> t_closest_bullet) const -> bool {
        if (!t_closest_bullet.has_value()) {
            return false;
        }
        const auto bullet_middle_x = t_closest_bullet.value().left + t_closest_bullet.value().width / 2.0f;
        const auto bullet_middle_y = t_closest_bullet.value().top + t_closest_bullet.value().height / 2.0f;
        const auto enemy_middle_x = get_reduced_texture_rect().left + get_reduced_texture_rect().width / 2.0f;
        const auto enemy_middle_y = get_reduced_texture_rect().top + get_reduced_texture_rect().height / 2.0f;
        auto delta = static_cast<float>(t_screen_dimensions.x) / 4.5f;
    ...
    auto avoiding_enemy::calculate_direction(const sf::Vector2u t_screen_dimensions,
