...
auto powerup::apply_effect(const std::unique_ptr<player_ship>& t_ship) const
    -> void {
    switch (m_type) {
...
auto powerup::generate_powerup(
    const sf::Vector2f t_pos, sf::Clock& t_powerup_clock,
    std::map<powerup_type, sf::Texture>& t_textures) -> std::optional<powerup> {
    if (std::floor(t_powerup_clock.getElapsedTime().asSeconds()) <=
        constants::pwu_spawn_interval) {
        return std::nullopt;
    }
...
auto powerup::generate_object(
    const powerup_type t_type, const sf::Vector2f t_pos,
    std::map<powerup_type, sf::Texture>& t_textures) -> powerup {
    switch (t_type) {
        case powerup_type::health: {
            auto health =
                powerup(powerup_type::health);  //random generated variety
        ...
        case powerup_type::rocket: {
            auto rocket =
                powerup(powerup_type::rocket);  //random generated variety
            rocket.set_texture(t_textures[powerup_type::rocket]);
            rocket.set_position(t_pos);
            return rocket;
        }
        case powerup_type::temporary_speed: {
            auto temporary_speed = powerup(
                powerup_type::temporary_speed);  //random generated variety
            temporary_speed.set_texture(
                t_textures[powerup_type::temporary_speed]);
