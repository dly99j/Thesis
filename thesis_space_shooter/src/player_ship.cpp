...
namespace spsh {
player_ship::player_ship(const direction t_direction,
                         const player_details& t_player_details)
    : ship_base(t_direction, t_player_details.speed, t_player_details.lives),
      ...
      m_boosted_speed(t_player_details.boost_multiplier *
                      t_player_details.speed),
    ...
    for (std::size_t i = 0; i < t_player_details.bullets; ++i) {
        m_bullets.emplace(direction::up, m_bullet_speed,
                          projectile_type::rocket);
    }
}

auto player_ship::shoot(const std::optional<sf::FloatRect> t_nothing)
    -> std::optional<projectile> {
    if (t_nothing.has_value()) {
        std::cerr << "Fatal developer error: player_ship::shoot should never "
                     "receive a param value\n";
...
auto player_ship::handle_speed_boost() -> void {
    if (m_speed_boost_timer.getElapsedTime() < m_speed_boost_duration &&
        m_is_boost_active) {
        return;
    }
