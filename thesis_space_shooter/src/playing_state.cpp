...
playing_state::playing_state(sf::RenderWindow& t_window, const map t_map)
    : m_map(t_map),
      ...
      m_window(t_window),
      m_sound_player({sound_effect::player_rocket, sound_effect::player_hit,
    ...
    m_player->set_position({window_width / 2.0f - player_width / 2.0f,
...
auto playing_state::move_projectiles(const sf::Time t_delta_time) -> void {
    ...
    std::erase_if(
        m_projectiles, [size = m_window.getSize()](const projectile& proj) {
...
auto playing_state::detect_collision() -> void {
    ...
    for (auto& proj : m_projectiles) {
        if (m_player->get_reduced_texture_rect().intersects(
            ...
            proj.get_direction() == direction::down) {
...
auto playing_state::draw_game_over(game_state t_state) -> sf::RectangleShape {
    ...
    if (t_state == game_state::lost) {
        ...
        std::cerr
            << "Fatal error! draw_game_over called when the game is not over";
    ...
    main_menu_text.setPosition(
        ...
        button_y +
            (button_height - main_menu_text.getLocalBounds().height) / 2.f -
            5.f);
...
auto playing_state::get_closest_bullet_to_enemy()
    ...
    auto distance_sq = [this](const projectile& p) {
        return std::pow(this->m_enemy->get_position().x - p.get_position().x,
                        2) +
               std::pow(this->m_enemy->get_position().y - p.get_position().y,
                        2);
    };
...
auto playing_state::send_asteroid_if_needed() -> void {
    ...
    projectile generated(direction::down, constants::asteroid_speed,
                         projectile_type::asteroid);
...
auto playing_state::generate_random_coords(const sf::Vector2u t_size) const
    ...
    std::uniform_real_distribution<float> dist_x(
        0.0f, static_cast<float>(m_window.getSize().x) -
                  static_cast<float>(t_size.x));
    std::uniform_real_distribution<float> dist_y(
        0.0f, static_cast<float>(m_window.getSize().y) -
                  static_cast<float>(t_size.y));
